...
function t1(min, max) {
    ...
    document.querySelector('.out-1').textContent = number;
}
...
function t3(min = 0, max = 100) {
    ...
    document.querySelector('.out-3').textContent = randNum;
}
...
function t12(...args) {
    const argSum = args.reduce((prev, current) => {
        return prev + current;
    });
    out12.textContent = argSum;
}
